...
def adjustData(img, mask, flag_multi_class, num_class):
    if (flag_multi_class):
        ...
        # mask[mask > 0.5] = (mask[mask > 0.5]-0.5)/2 + 0.75
        # mask[mask <= 0.5] = mask[mask <= 0.5]/2
        #
        # mask[mask > 0.5] = 1
        # mask[mask <= 0.5] = 0
    return (img, mask)
...
def saveResult(save_path, npyfile, Pbar, flag_multi_class=False, num_class=2):
    fs = []
    for file in os.listdir(save_path.replace("/results", "")):
        if re.search(r'([a-zA-Z0-9\s_\\.\-\(\):])+(.jpg|.jpeg|.png)$', file):
            fs.append(file.replace(".png", ""))
    for item, f in zip(npyfile, fs):
        img = labelVisualize(num_class, COLOR_DICT, item) if flag_multi_class else item[:, :, 0]
        # img = cv2.resize(img,(512,512))
        io.imsave(os.path.join(save_path, "%s_predict.png" % (f)), img_as_uint(img))
        print("YOYOYOYO")
        Pbar.addition()
        print("heyheyheyhey")
