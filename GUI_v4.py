...
# class Process_bar(Progressbar):
#     def __init__(self, no_pic, *args, **kwargs):
#         self.no_pic = 0
#         super().__init__(self,*args,**kwargs)
#     # def __init__(self, no_pic,master,maximum,value):
#     #     super().__init__(master,maximum,value)
#     #     self.no_pic = 0
#
#     def addition(self):
#         print("hey")
#         if self.no_pic < self["maximum"]:
#             self.no_pic += 1
#             self["value"] = self.no_pic
#             self.after(50,self.addition)
#             print(self.no_pic)
...
class Process_bar(Progressbar):
    ...
    def __init__(self, newWin):  # , newWin, maximum,value,no_pic):
        # super().__init__(newWin,maximum,value)
        super().__init__(newWin)

    def addition(self):
        # print("no_pic：",self.no_pic)
        self.no_pic += 1
        # print("no_pic++：",self.no_pic)
        pb["value"] = self.no_pic
        # print("pb['value']: ", pb["value"])
        if self.no_pic < pb["maximum"]:
            pb.after(50, self.addition)
...
def start():
    ST = threading.Thread(target=start_Function)
    ST.start()
...
def start_Function():
    ...
    try:
        ...
        '''process bar'''
        newindow = tk.Tk()
        # pb = Process_bar(newindow, total_page,0,0)
        pb = Process_bar(newindow)
        pb.pack()

        close_btn = tk.Button(newindow, text="OK")
        close_btn.pack()

        pb["value"] = 0
        pb["maximum"] = total_page
        predict.predict_GUIv3(pb)  # loading()
...
main_Thread = threading.Thread(target=(lambda x: x.mainloop())(window))
main_Thread.start()
